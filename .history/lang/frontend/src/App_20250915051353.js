...
import { Upload, Mic, MicOff, Download, Play, Pause, FileText, Globe, Sparkles, Zap, Volume2, AlertCircle, CheckCircle } from 'lucide-react';
...
const App = () => {
  ...
  const startRecording = async () => {
    try {
      ...
      showError('Error accessing microphone. Please check permissions.');
    }
  };
  ...
  const processAudio = async () => {
    if (!audioBlob) {
      showError('No audio available to transcribe!');
      return;
    }
    ...
    try {
      ...
      showError('Error processing audio. Make sure the Flask server is running on http://127.0.0.1:5000');
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `meeting_summary_${new Date().toISOString().slice(0,19).replace(/:/g,'-')}.txt`;
        ...
        showError('Error downloading report: ' + (errorData.error || 'Unknown error'));
      }
    } catch (error) {
      console.error('Error:', error);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 relative overflow-hidden">
      ...
      {error && (
        <div className="fixed top-4 right-4 bg-red-500/90 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2 backdrop-blur-sm">
      ...
      {success && (
        <div className="fixed top-4 right-4 bg-green-500/90 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2 backdrop-blur-sm">
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-2">Supported formats: MP3, WAV, M4A (Max: 50MB)</p>
                    </div>
                    <input
                      ref={fileInputRef}
                      type="file"
        ...
        {transcriptText && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="text-center">
              <button
                className="px-10 py-4 bg-gradient-to-r from-violet-500 to-purple-600 text-white font-bold rounded-2xl hover:scale-105 transition-all duration-300 flex items-center space-x-3 mx-auto shadow-lg shadow-violet-500/30 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
